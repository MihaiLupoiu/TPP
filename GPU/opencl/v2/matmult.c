...
int main (int argc, char *argv[])
{
  ...
  // Asignar memoria a local_work_size
  ...
  // especificar los threads locales
  local_work_size = (size_t*) malloc(DIMENSIONS * sizeof(size_t));

  global_work_size[0] = nA;
  global_work_size[1] = mB;

  //Tamaño de local_work_size
  ...
  // Ahora le especificamos el local_work_size; tiene que coincidir con BOCKSIZE en OpenCL.
  work_dim = DIMENSIONS;
  ...
  // Print something.
  //printMatrixCustom(h_A,SQRSIZE,SQRSIZE);
  //printMatrixCustom(h_B,SQRSIZE,SQRSIZE);
  //printMatrixCustom(h_C,SQRSIZE,SQRSIZE);
