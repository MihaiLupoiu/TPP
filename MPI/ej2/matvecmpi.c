...
void parmatvec(int N,int n,int p,int b,double *A, double *v, double *w)
{
  ...
  if (me>0) {
    MPI_Isend(v+b,b,MPI_DOUBLE,me-1,0,MPI_COMM_WORLD,&requestEnvio);
  }
  /* comunicación con vecino inferior */
  if (me<np-1) {
    MPI_Isend(v+n,b,MPI_DOUBLE,me+1,0,MPI_COMM_WORLD,&requestEnvio);
  }

  for(i=0; i<n; i++) {
    w[i+b] = 0.0;
    li = i+p-b<p? p: i+p-b;  /* limite inferior */ // para que siempre empiece en p o en el lugar que le toque.
    ls = i+p+b>p+n-1-1? p+n-1: i+p+b;  /* limite superior */ //para acabar en p+n-1
    for(j=li; j<=ls; j++) {
